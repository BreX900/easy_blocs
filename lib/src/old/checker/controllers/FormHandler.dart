...
class FormHandler {
  ...
  FormHandler({
    List<Validator> preValidators,
    List<Validator> postValidators,
  })  : this._preValidators = preValidators ?? [],
        this._postValidators = postValidators ?? [];
  ...
  Future<bool> _validate(List<Validator> validators) async {
    for (var validator in validators) {
      if (!await validator()) return false;
    }
    return true;
  }
